...
@interface AppstoreUnrestrictSettingsController : PSListController {
	UILabel* _label;
	UILabel* underLabel;
}
- (void)HeaderCell;
...
@implementation AppstoreUnrestrictSettingsController
- (id)specifiers {
	if (!_specifiers) {
        ...
        [specifiers addObject:spec];
		spec = [PSSpecifier emptyGroupSpecifier];
        [spec setProperty:@"Appstore Unrestrict © 2018" forKey:@"footerText"];
        [specifiers addObject:spec];
...
- (void)love
{
	...
	[twitter setInitialText:@"#AppstoreUnrestrict by @ijulioverne is cool!"];
	if (twitter != nil) {
...
- (void)reset
{
	...
	notify_post("com.julioverne.appstoreunrestrict/Settings");
}
...
- (void)HeaderCell
{
	...
	CGRect frame = CGRectMake(0, 20, width, 60);
		...
		[underLabel setText:@"No Download Limit AppStore"];
...
{
	[super loadView];
	self.title = @"Appstore Unrestrict";	
